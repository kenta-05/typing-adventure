  ...
  build: function (seq) {
    ...
    const merge_parents = (parents) => {
      ...
      parents
        ...
        .filter(([parent1, replaced]) => {
          ...
          replaced.push(
            parents.find((parent0) => node_equals(parent0, parent1))
          );
        ...
        .forEach(([parent, replaced_parent]) => {
          ...
          parent._merged = true;
          parent._children?.forEach((sibling) => {
            sibling._parents.splice(sibling._parents.indexOf(parent), 1);
          });
          parent._parents?.forEach((granpa) => {
            ...
            replaced_parent[0]._parents.includes(granpa) ||
              replaced_parent[0]._parents.push(granpa);
          });
          replaced_parents.includes(replaced_parent[0]) ||
            replaced_parents.push(replaced_parent[0]);
        });

      // 残った親は、その親がマージできる可能性があるので繰り返す。
      replaced_parents
        .filter((e) => e._parents !== undefined)
        .forEach((e) => {
    ...
    const add_chains = (parent, key) => {
      ...
      Array.from(key).forEach((k) => {
        ch =
          ch.find_child(k) ??
          Object.create(this._chain_proto).set(k, ch).store(this._chains);
      });
      return ch;
    };
    ...
    while (
      ...
      this._char_keys_table
        .filter((ckeys) =>
          this._seq.substring(n.p).startsWith(ckeys.char)
        ) /*.sort((e0, e1) => e0.char.length - e1.char.length)*/
        .forEach((ckeys) => {
          const keys =
            typeof ckeys.keys === "function"
              ? ckeys.keys.bind(this)(this._seq.substring(n.p))
              : ckeys.keys;
  ...
  next: function (key) {
    ...
    if (seq_ptr_cur0 !== this._seq_ptr_cur) {
      ...
      this._autocomplete_registered.forEach((a) => {
        if (
          this._seq
            .substring(this._seq_ptr_cur + (a.seq_ptr_d ?? 0))
            .match(a.seq_pattern)
        ) {
  ...
  _char_keys_table: [
    // 関数で入力するキーを決定するもの
    {
      ...
      keys: function (seq) {
        ...
        if (
          seq.length === 1 ||
          key_startsWith(seq.substring(1), ["a", "i", "u", "e", "o", "n", "y"])
        ) {
          // nnと2つ打つ必要がある。
          return ["nn", "xn"];
        } else {
